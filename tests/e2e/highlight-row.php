<?php
/**
 * Group-page row highlight: when /pronostico/g/<group>/?as=<share> resolves
...
/* ------------------------------------------------------------------------ */
Mantia_E2E::assert_http_ok( '/pronostico/g/' . $group_token . '/', array( 'mantia-board-row' ) );
...
Mantia_E2E::assert_http_ok(
	'/pronostico/g/' . $group_token . '/?as=' . $bob_share,
...
$resp = wp_remote_get( home_url( '/pronostico/g/' . $group_token . '/?as=' . $bob_view ) );
$body = is_wp_error( $resp ) ? '' : (string) wp_remote_retrieve_body( $resp );
