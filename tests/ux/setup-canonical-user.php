...
// 2. Create a libertadores-prueba penca and join Alice.
$group_id = Mantia_Repository::create_group( 'P_UX_TEST', '', '', 'libertadores-prueba' );
if ( $group_id <= 0 ) {
...
// 3. Pick the first upcoming match in libertadores-prueba and have
...
$matches = Mantia_Repository::upcoming_matches_for_competition( 'libertadores-prueba', 24 * 365 );
if ( empty( $matches ) ) {
	echo "ERROR: no upcoming matches in libertadores-prueba\n";
	exit( 1 );
}
...
$comp_url_as  = Mantia_Repository::competition_view_url( 'libertadores-prueba' ) . '?as=' . rawurlencode( $share_token );
$comp_url_an  = Mantia_Repository::competition_view_url( 'libertadores-prueba' );
